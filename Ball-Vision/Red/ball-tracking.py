...
while(True):
    ...
    #captured_frame_hsv_red = cv.inRange(captured_frame_hsv, np.array([0, 115, 42]), np.array([180,210,246]))

    # Thresholding the  image to keep only the red pixels (close up)
    #captured_frame_hsv_red = cv.inRange(captured_frame_hsv, np.array([0, 86, 34]), np.array([178,210,256]))

    # Thresholding the  image to keep only the red pixels (halfway)
    captured_frame_hsv_red = cv.inRange(captured_frame_hsv, np.array([0, 128, 49]), np.array([180,219,256]))

    # Thresholding the  image to keep only the red pixels (all distances)
    #captured_frame_hsv_red = cv.inRange(captured_frame_hsv, np.array([102, 70, 55]), np.array([117,208,256]))
    ...
    circles = cv.HoughCircles(captured_frame_hsv_red, cv.HOUGH_GRADIENT_ALT, 1, captured_frame_hsv_red.shape[0] / 8, param1=300, param2=0.9, minRadius=10, maxRadius=300)
